...
parser.add_argument("--tokens", default='ON', type=str,
                    help='Value [ON/OFF], activate or deactivate token system'),  # по умолчанию включено (ON)
...
if not os.path.exists(os.path.join(BASE_DIR,
                                   content_dir)):
    # если нету папки 'UPLOADS', в которой будут храниться все загрузки, создаем ее
