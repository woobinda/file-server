...
def make_folder_for_file(file_hash):
    # функция создает папку и возвращает путь к директории куда будет перемещен загружаемый файл
    path = os.path.join(app.config['UPLOAD_FOLDER'], file_hash[0], file_hash[1])  # если нужной нам папки не существует
    if os.path.exists(path) == False:
        os.makedirs(os.path.join(app.config['UPLOAD_FOLDER'], file_hash[0], file_hash[1]))
    return path
...
def upload_file():
        ...
        if file and token:
            ...
            if tokens == 'ON':
                if token != get_token(file_name, file_size, client_ip,
                                      args.secret):  # сверяем пришедшей токен со сгенерированым
                    abort(404)
            make_folder_for_file(file_hash)  # создаем папку для загрузки файла
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], file_hash[0], file_hash[1], file_hash))  # сохраняем
            confirm_token = get_token(file_name, file_size, client_ip, args.secret,
                                      file_hash)  # токен для подтверждения корректной загрузки
            response = jsonify({
                "file_name": file_name,
                "file_size": file_size,
                "file_hash": file_hash,
    ...
    if request.method == 'GET' and tokens == 'ON':  # если активирован режим tokens
        return '''
		<!doctype html>
	    <title>Locked</title>
...
if __name__ == '__main__':
    app.run(host='127.0.0.1', port=args.port,
            threaded=True)  # host='0.0.0.0'; threaded=True - рекамендация по запуску приложений без настроенного WSGI
