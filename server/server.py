...
def sha1(stream):
    """
    The function returns the md5 hash, based on the parameters of the uploaded file.
    """
...
def upload_file():
    ...
    If token is successful - download a file and return a response with confirm token.
    """
    if request.method == 'POST':
        ...
        if file and token:
            ...
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], file_hash[0], file_hash[1], file_hash))
...
def get_file(file_hash):
    ...
    """ 
    folder = os.path.join(app.config['UPLOAD_FOLDER'], file_hash[0], file_hash[1])
    if folder:
